...
if (process.argv.length !== 3 || !['--write', '--check'].includes(mode)) {
  console.error('Uso: node scripts/sync-role-architect-skill.cjs <--write|--check>');
  process.exit(1);
}

const root = path.resolve(__dirname, '..');
const sourceDirectory = path.join(root, '.agents/skills/role-architect');
const targetDirectory = path.join(root, '.claude/skills/role-architect');
const marker = '<!-- GENERATED FROM .agents/skills/role-architect/SKILL.md; DO NOT EDIT -->';
...
if (!synchronized) {
  console.error('A cópia Claude está ausente ou desatualizada. Execute: node scripts/sync-role-architect-skill.cjs --write');
  process.exit(1);
}
